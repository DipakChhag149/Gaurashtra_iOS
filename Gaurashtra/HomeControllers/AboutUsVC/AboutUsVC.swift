...
class AboutUsVC: GaurashtraBaseVC,WKUIDelegate,WKNavigationDelegate {
  ...
  func getDevice()
    ...
    private func callAboutUsWebService()
    {
        
        let dictImg:[String : Any] = ["image" : UIImage(),
                                      "imageName" : "uploaded_file"]
        TANetworkManager.sharedInstance.requestMultiPart(withServiceName: kAboutUs, requestMethod: .post, requestImages: [dictImg], withProgressHUD: true, withProgessHUDTitle: "Please Wait", requestVideos: [String:  Any](), requestData: dictParams, completionClosure: {[weak self] (response: Any?, error: Error?, errorType: ErrorType, statusCode: Int?) in
            ...
            if errorType == .requestSuccess
            {
                ...
                if Int.getInt(dictResponse["success"]) == 1
                {
                    ...
                    let dictData = kSharedInstance.getDictionary(result["aboutUs"])
