...
class TermsAndConditionsVC: GaurashtraBaseVC,WKUIDelegate,WKNavigationDelegate  {
    ...
    override func viewDidLoad() {
        ...
        {
            self.callTermsConditionsWebService()
        }else{
    ...
    //#MARK:-----------callTermsConditionsWebService---------
    private func callTermsConditionsWebService()
    {
        
        let dictImg:[String : Any] = ["image" : UIImage(),
                                      "imageName" : "uploaded_file"]
        TANetworkManager.sharedInstance.requestMultiPart(withServiceName: kTermsConditions, requestMethod: .post, requestImages: [dictImg], withProgressHUD: true, withProgessHUDTitle: "Please Wait", requestVideos: [String:  Any](), requestData: dictParams, completionClosure: {[weak self] (response: Any?, error: Error?, errorType: ErrorType, statusCode: Int?) in
            ...
            if errorType == .requestSuccess
            {
                ...
                if Int.getInt(dictResponse["success"]) == 1
                {
                    ...
                    let dictData = kSharedInstance.getDictionary(result["termsConditions"])
